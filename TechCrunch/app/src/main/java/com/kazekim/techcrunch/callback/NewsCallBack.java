...
public class NewsCallBack extends JHCallBack<ResultDao> {
    ...
    public void loadNewsList(NewsListCallBackListener listener)
    {
        ...
        HttpManager httpManager = new HttpManager(new JHDeserializer<NewsDao>(NewsDao.class));
    ...
    private void handleNewsList(ResultDao<NewsDao> dao)
    {
        if(HTTPStatus.fromString(dao.getStatus()) == HTTPStatus.ok) {
            if (newsListCallBackListener != null)
                newsListCallBackListener.onNewsListLoadSuccess(mode, dao.getDataList());
        }else {
